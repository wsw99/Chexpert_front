...
export default function UploadImage(props) {
  ...
  const [fileList, setFileList] = useState([]);      // antd 受控 fileList
  const [generateHeatmap, setGenerateHeatmap] = useState(true);
  const [threshold, setThreshold] = useState(0.5);
  const [topK, setTopK] = useState(null);
  const [loading, setLoading] = useState(false);
  const [internalResults, setInternalResults] = useState(null); // ★ 本地保存结果并显示
  ...
  const onChange = (info) => {
    ...
    setInternalResults(null); // 重新选文件时清空旧结果
  };
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (!res?.success) {
        ...
        setInternalResults(res); // ★ 保存并触发下方 HeatmapDisplay 渲染
      }
  ...
  return (
    <Card className="upload-card" title="上传影像进行分析">
      <Space direction="vertical" style={{ width: '100%' }}>
        ...
        {/* ★ 内联显示分析结果（即使父组件没接回调也能看到） */}
        {internalResults ? (
          <HeatmapDisplay analysisResults={internalResults} isAnalyzing={loading} />
        ) : null}
      </Space>
    </Card>
  );
